...
export const CommandPaletteSelect: React.FC<CommandPaletteSelectProps> = ({
  ...
  return (
    <div className="relative w-full" ref={containerRef} id={id}>
      {label && (
        <label className="block text-[11px] font-semibold text-zinc-600 dark:text-zinc-400 mb-1 truncate">
          {label}
        </label>
      )}
      ...
      {isOpen && (
        <div
          className={`absolute z-[100] ${openUpward ? 'bottom-full mb-1.5' : 'top-full mt-1.5'} ${align === 'right' ? 'right-0' : 'left-0'} min-w-[280px] sm:min-w-[320px] max-w-[420px] w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-2xl overflow-hidden animate-in fade-in-50 zoom-in-95 duration-100 ring-1 ring-black/5 dark:ring-white/10`}
        >
          {/* Search */}
          <div className="flex items-center gap-2 px-3 py-2 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/50">
